...
from platform_enforcement import apply_platform_state
...
def compose_v2(root:Path,repository:str,revision:str,default_branch:str,observed_at:str,evidence_bundle:dict|None=None)->dict:
    ...
    if evidence_bundle is not None:
        ...
        portrait=apply_platform_state(portrait,evidence_bundle)
        ...
        for key in ('runtime_evidence','artifact_provenance_evidence','platform_enforcement_evidence'):
            for item in portrait.get(key,{}).get('unknowns',[]):
                row={'dimension':key,'detail':item}
                if row not in portrait['overall_portrait']['explicit_unknowns']:
                    portrait['overall_portrait']['explicit_unknowns'].append(row)
